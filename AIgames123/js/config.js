...
const SECURITY = {
    ...
    // Game-file URL allowlist: file_url MUST start with one of these prefixes.
    // Rejects javascript:, data:, http:, and URLs pointing at the parent app's
    // own origin (sandbox-escape vector). Two prefixes are allowed:
    //   1. Direct Supabase storage URL (new uploads before CDN migration)
    //   2. The cdn.aigames123.com Cloudflare Worker (cached / migrated files)
    ALLOWED_GAME_URL_PREFIXES: [
        `${SUPABASE_URL}/storage/v1/object/public/`,
        'https://cdn.aigames123.com/',
    ],
 
    // CDN base URL. New uploads return Supabase storage URLs; we rewrite
    // them to this CDN before saving to the DB, so all DB rows reference
    // the cached origin instead of hitting Supabase egress on every load.
    CDN_BASE: 'https://cdn.aigames123.com',
...
// Rewrite a Supabase Storage public URL to use our CDN domain.
// Idempotent: calling it on an already-CDN URL leaves it untouched.
// If the URL doesn't match the Supabase storage pattern at all, it's
// returned as-is (e.g. external GitHub avatar URLs).
function toCdnUrl(url) {
    if (typeof url !== 'string' || !url) return url;
    const supabaseStoragePrefix = `${SUPABASE_URL}/storage/v1/object/public/game-files/`;
    if (url.startsWith(supabaseStoragePrefix)) {
        return SECURITY.CDN_BASE + '/' + url.slice(supabaseStoragePrefix.length);
    }
    return url;
}
...
function assertSafeGameUrl(url) {
    ...
    // Must match one of the allowed storage prefixes (Supabase OR CDN).
    const allowed = SECURITY.ALLOWED_GAME_URL_PREFIXES.some(p => url.startsWith(p));
    if (!allowed) {
